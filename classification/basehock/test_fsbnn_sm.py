...
# Apply FSBNN-ws on Basehock data
import torch
...
curr_dir = "/workspace/classification/dataset-stg-6"
...
target_classes = {v: k for k, v in label_encoder_name_mapping.items()}  # Invert the dictionary
# Convert label encoder mapping to a list of class names
target_class_names = [str(target_classes[i]) for i in range(len(target_classes))]
...
for k in range(1, 2):
    ...
    print(classification_report(y_test.cpu().numpy(), pred2.cpu().numpy(), target_names=target_class_names))
...
# curr_dir = "/Users/Akanksha Mishra/Documents/GitHub/sbnn/classification/dataset-stg-6"
# code = "fsbnn_samplemean"
# #------------------------------------------------------------------------------------------------------
# # Create a simple dataset
# mat = scipy.io.loadmat(f'{curr_dir}/BASEHOCK.mat')
...
#     sorted, indices = torch.sort(one1_w,0, descending=True)
#     print('features selected in the first layer: {}'.format(indices[0:10]))
#     torch.save(sorted, f"{curr_dir}/{code}/ws_sorted_run10.pt")
#     torch.save(indices, f"{curr_dir}/{code}/ws_fsbnn_indices_run10.pt")
#     np.save(f"{curr_dir}/{code}/ws_fsbnn_run10",indices)
...
# plt.tight_layout()
# plt.savefig(f"{curr_dir}/{code}/Training_Loss3")
# plt.show()
...
# plt.legend()
# plt.tight_layout()
# plt.savefig(f"{curr_dir}/{code}/Train_Test_Accuracy3")
# plt.show()
