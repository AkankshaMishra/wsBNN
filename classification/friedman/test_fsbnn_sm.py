...
from sparse_bnn_classification_vhd_l1 import FeatureSelectionBNNClassification
...
for k in range(1, 4):
    ...
    print('gamma_w final: {}'.format(net.module.l1.gamma_w))
    print('w final: {}'.format(net.module.l1.w))
    ...
    torch.save(sorted, f"{curr_dir}/{code}/wtheta_sorted_run{k}.pt")
    torch.save(net.module.l1.w, f"{curr_dir}/{code}/w_run{k}.pt")
    ...
    train_precision = precision_score(y_train.cpu().numpy(), pred.cpu().numpy(), average='weighted', zero_division=0.0)
    train_recall = recall_score(y_train.cpu().numpy(), pred.cpu().numpy(), average='weighted', zero_division=0.0)
    train_f1 = f1_score(y_train.cpu().numpy(), pred.cpu().numpy(), average='weighted', zero_division=0.0)
    ...
    test_precision = precision_score(y_test.cpu().numpy(), pred2.cpu().numpy(), average='weighted', zero_division=0.0)
    test_recall = recall_score(y_test.cpu().numpy(), pred2.cpu().numpy(), average='weighted', zero_division=0.0)
    test_f1 = f1_score(y_test.cpu().numpy(), pred2.cpu().numpy(), average='weighted', zero_division=0.0)
...
with open(f"{curr_dir}/{code}/metrics.txt", "a") as metrics_file:
    metrics_file.write(f"Lambda: 0.1\n")
