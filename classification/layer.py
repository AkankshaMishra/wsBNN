...
class SpikeNSlabLayer1(nn.Module):
    """
    Spike-and-Slab layer with Gumbel-Softmax approximation or truncated gaussian.
    
    Paper ↔ Code mapping:
    ----------------------
    w^{(0)}_{ij}   : self.w  (sampled weights)
    z_i            : self.gamma_w  (Bernoulli gate, relaxed via Gumbel-Softmax)
    lambda_0       : lambda0 (prior inclusion probability)
    tau_0^2        : sigma_w^2 (slab variance, from w_rho)
    delta_0        : enforced by gamma_w (when 0, weight = 0)
    phi_i (sigmoid): sigmoid(self.w_theta) (posterior inclusion prob)

    For hidden layers:
    w^{(l)}_{ij}, b^{(l)}_i ~ N(0, tau_1^2)
      → Implemented in NormalLayer using (w_mu, w_rho), (b_mu, b_rho)
    """

    def __init__(self, input_dim, output_dim, rho_prior, device, rho0=-6., lambda0=0.99, flag=False):
        # initialize layers
        super(SpikeNSlabLayer1, self).__init__()
        ...
        # initialize mu, rho and theta parameters for layer's weights
        # Slab Gaussian parameters
        self.w_mu = nn.Parameter(torch.Tensor(input_dim).uniform_(-0.6, 0.6))
        self.w_rho = nn.Parameter(torch.Tensor(input_dim).uniform_(rho0, rho0))

        # initialize mu, rho and theta parameters for layer's biases, theta = logit(phi)
        # Bias Gaussian parameters
        self.b_mu = nn.Parameter(torch.Tensor(output_dim).uniform_(-0.6, 0.6))
        self.b_rho = nn.Parameter(torch.Tensor(output_dim).uniform_(rho0, rho0))

        # Logits for Bernoulli gates (z_i) → λ_0 prior controls initialization
        self.w_theta = nn.Parameter(logit(torch.Tensor(input_dim).uniform_(lambda0, lambda0)))
        self.b_theta = nn.Parameter(logit(torch.Tensor(output_dim).uniform_(lambda0, lambda0)))
        ...
        # initialize weight samples (these will be calculated whenever the layer makes a prediction)
        self.gamma_w = None # z_i for weights
        self.gamma_b = None
    ...
    def forward(self, X, temp, phi_prior, flag=False):
        ...
        self.w = self.w.reshape([X.size()[1],1])
        self.w = self.w.expand(-1, self.output_dim)
        
        output = torch.mm(X, self.w) + self.b.expand(X.size()[0], self.output_dim)

        return output

class SpikeNSlabLayerModified(nn.Module):
    """
    Spike-and-Slab layer with Gumbel-Softmax approximation or truncated gaussian.
    ...
    def __init__(self, input_dim, output_dim, rho_prior, device, rho0=-6., lambda0=0.99, flag=False):
        # initialize layers
        super(SpikeNSlabLayerModified, self).__init__()
        ...
        # Logits for Bernoulli gates (z_i) → λ_0 prior controls initialization
        if flag:
            self.w_theta = nn.Parameter(torch.rand(input_dim))
            self.b_theta = nn.Parameter(torch.rand(output_dim))
        else:
            self.w_theta = nn.Parameter(logit(torch.Tensor(input_dim).uniform_(lambda0, lambda0)))
            self.b_theta = nn.Parameter(logit(torch.Tensor(output_dim).uniform_(lambda0, lambda0)))
    ...
    def forward(self, X, temp, phi_prior, flag=False):
        ...
        if flag:
            w_sigma_ve = 1
            b_sigma_ve = 1
            epsilon_wt = Normal(0, 1).sample(self.w_theta.shape).to(self.device)
            epsilon_bt = Normal(0, 1).sample(self.b_theta.shape).to(self.device)
            self.gamma_w = truncated_gaussian(self.w_theta, w_sigma_ve, epsilon_wt, hard=True)
            self.gamma_b = truncated_gaussian(self.b_theta, b_sigma_ve, epsilon_bt, hard=True)
            w_phi = gaussian_cdf(self.w_theta / w_sigma_ve)
            b_phi = gaussian_cdf(self.b_theta / b_sigma_ve)
        else:
            #Sample Bernoulli gates (shared per feature / per bias)
            u_w = torch.rand(self.w_theta.shape).to(self.device)
            u_b = torch.rand(self.b_theta.shape).to(self.device)
            self.gamma_w = gumbel_softmax(self.w_theta, u_w, temp, hard=True)
            self.gamma_b = gumbel_softmax(self.b_theta, u_b, temp, hard=True)
        ...
        w_phi_exp = w_phi.view(-1, 1).expand(-1, self.output_dim)
        # KL-divergence terms
        kl_w = w_phi_exp * (torch.log(w_phi_exp) - torch.log(phi_prior)) + \
            (1 - w_phi_exp) * (torch.log(1 - w_phi_exp) - torch.log(1 - phi_prior)) + \
            w_phi_exp * (torch.log(sigma_prior) - torch.log(sigma_w) +
                     0.5 * (sigma_w ** 2 + self.w_mu ** 2) / sigma_prior ** 2 - 0.5)
        ...
        # Reparameterization trick for Gaussian slab weights 
        epsilon_w = Normal(0, 1).sample(self.w_mu.shape).to(self.device)
        epsilon_b = Normal(0, 1).sample(self.b_mu.shape).to(self.device)

        W_sample = self.w_mu + sigma_w * epsilon_w   # [in_features, out_features]
        b_sample = self.b_mu + sigma_b * epsilon_b  # [out_features]

        # Apply feature-level gates: expand gamma_w → [in_features, out_features]
        mask_w = self.gamma_w.view(-1,1).expand(-1, self.output_dim)
        self.w = W_sample * mask_w
        self.b = b_sample * self.gamma_b
