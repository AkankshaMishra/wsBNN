...
import time
import sys
sys.path.insert(0, '/Users/Akanksha Mishra/Documents/files-from-server/wsbnn-sbnn-hpc-19-sep/regression')
...
print(device)
curr_dir = "/Users/Akanksha Mishra/Documents/files-from-server/wsbnn-sbnn-hpc-19-sep/regression/friedman1"
code = "fsbnn_sm"
...
class FSBNNWrapper:
    ...
    def fit(self, X_train, y_train):
        pass  # No need to train again, model is already trained

    def predict(self, X):
        X = torch.Tensor(X).to(self.device)
        _, _, _, preds = self.model.module.sample_elbo(X, torch.zeros(X.shape[0], dtype=torch.long).to(self.device), 30, self.temp, self.phi_prior, self.num_batches)
        return preds.mean(dim=0).cpu().detach().numpy()
...
for k in range(1, 2):
    ...
    mse2, bias2, var2 = bias_variance_decomp(
    estimator=fsbnn_wrapper,  # Pass the wrapped FSBNN model as the estimator
    X_train=x_train_cpu,
    y_train=y_train_cpu,
    X_test=x_test_cpu,
    y_test=y_test_cpu,
